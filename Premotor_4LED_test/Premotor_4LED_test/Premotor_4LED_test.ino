...
SoftwareSerial HC_Serial(19,18);
...
void setup() {
  // communication with the host computer
    ...
    while (serialByte !='1')
    {
        if ( HC_Serial.available() )   {  serialByte = Serial.read();  }
    }  
...
void loop() {
   ...
   if ( HC_Serial.available() )   {  Serial.write( HC_Serial.read() );  }

   // listen for user input and send it to the HC-05
   
   if ( Serial.available() )   {  HC_Serial.write( Serial.read() );  }
}
